...
object FrameConverter {
    fun convertToImageBitmap(frame: Frame, actualFramePixelFormat: Int): ImageBitmap? {
        return convertDirectToSkia(frame, actualFramePixelFormat)
    }

    private fun convertDirectToSkia(frame: Frame, actualFramePixelFormat: Int): ImageBitmap? {
        ...
        val skiaImage: Image? = when (actualFramePixelFormat) {
            ...
            avutil.AV_PIX_FMT_BGR24 -> {
                ...
                val bgrActualStride =
                    if (frame.imageStride > 0) frame.imageStride else calculatedBgrStride
                ...
                println(
                    "FrameToImageBitmapConverter (direct): Format ${
                        getPixelFormatName(
                            actualFramePixelFormat
                        )
                    } is not supported by this direct converter. Returning null."
                )
        ...
        if (finalResultBitmap != null) {
            println(
                "FrameToImageBitmapConverter (direct): Successfully converted format ${
                    getPixelFormatName(
                        actualFramePixelFormat
                    )
            ...
            println(
                "FrameToImageBitmapConverter (direct): Failed to convert format ${
                    getPixelFormatName(
                        actualFramePixelFormat
                    )
                }. Total time: $totalDuration ms"
    ...
    private fun createSkiaImageFromByteBuffer(
        ...
        val skiaExpectedRowBytes =
            width * bytesPerPixel // Skia expects this stride for the compact byte array
        ...
        try {
            if (sourceRowBytes == skiaExpectedRowBytes) {
                ...
                for (y in 0 until height) {
                    ...
                    sourceBuffer.get(
                        byteArrayForSkia,
                        y * skiaExpectedRowBytes,
                        skiaExpectedRowBytes
                    ) // Copy one compact row to target
                }
            }
        } catch (e: Exception) {
    ...
    private fun convertBgrToBgra(
        ...
        try {
            ...
            val minBytesNeededInSource =
                (height - 1) * bgrSourceStride + (width * 3) // Last pixel of last relevant row
            ...
            for (y in 0 until height) {
                val sourceRowStartOffset =
                    y * bgrSourceStride // Start of current row in source BGR buffer
                val targetRowStartOffset =
                    y * (width * 4)   // Start of current row in target BGRA array (compact)
